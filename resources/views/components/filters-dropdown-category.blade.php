...
<div class="relative" id="filterDropdownWrapper">
    ...
    <div id="filterDropdown"
        ...
        <div class="space-y-1">
            <a href="{{ route($routeName, array_merge(request()->except('industry'), ['status' => request('status', 'published')])) }}"
                class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm transition
        {{ !request('industry') ? 'bg-acmi-blueprimer/10 text-acmi-blueprimer font-medium' : 'text-gray-600 hover:bg-gray-50' }}">
                All Categories
            </a>
            @foreach ($categories as $cat)
                <a href="{{ route($routeName, array_merge(request()->except('industry'), ['industry' => $cat->name, 'status' => request('status', 'published')])) }}"
                    class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm transition
        {{ request('industry') == $cat->name ? 'bg-acmi-blueprimer/10 text-acmi-blueprimer font-medium' : 'text-gray-600 hover:bg-gray-50' }}">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
