...
<div class="relative" id="filterDropdownWrapper">
    ...
    <div id="filterDropdown"
        ...
        <div class="space-y-1">
            <a href="{{ route($routeName, array_merge(request()->except('category'), ['status' => request('status', 'published')])) }}"
            ...
            @foreach ($categories as $cat)
                <a href="{{ route($routeName, array_merge(request()->except('category'), ['category' => $cat->name, 'status' => request('status', 'published')])) }}"
                    class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm transition
