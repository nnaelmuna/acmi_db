    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-7">
        @forelse ($products as $item)
            <div class="group relative bg-white rounded-xl shadow-sm p-4 border border-gray-200 transition hover:shadow-md">
                ...
                <a href="#"
                    class="block w-full text-center mt-4 bg-[#4155C6] text-white py-2.5 rounded-lg font-medium transition hover:bg-[#3444a1]">Detail</a>
            </div>
        @empty
            <div
...
@push('scripts')
    <script>
        ...
        // Fungsi buat buka modal delete product
        function openDeleteModal(deleteUrl, message) {
            // 1. Set action form rahasia ke URL yang bener
            const form = document.getElementById('delete-item-form');
            form.action = deleteUrl;

            // 2. Munculin konfirmasi browser (atau kalau lu punya modal custom, panggil di sini)
            if (confirm(message)) {
                form.submit(); // Ini yang bikin refresh dan gak muncul 404/JSON
            }
        }
