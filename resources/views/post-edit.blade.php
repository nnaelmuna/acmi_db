...
@section('content')
    ...
    <form id="postForm" action="{{ route('post.update', $post) }}" enctype="multipart/form-data" method="POST" class="flex flex-col gap-8 pb-20">
        @csrf
        @method('PUT')
        ...
        <div class="w-full lg:w-[450px]">
            ...
            <div id="dropZone" onclick="document.getElementById('imageInput').click()" class="group relative flex h-[280px] w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-white transition hover:bg-gray-50">
                <img id="imagePreview" src="{{ $post->image ? asset('storage/' . $post->image) : '' }}" alt="Preview" class="absolute inset-0 {{ $post->image ? '' : 'hidden' }} h-full w-full rounded-2xl object-cover">
                <button id="removeImageBtn" type="button" onclick="removeImage(event)" class="absolute right-3 top-3 z-10 {{ $post->image ? 'flex' : 'hidden' }} h-8 w-8 items-center justify-center rounded-full bg-red-500 text-white shadow-md hover:bg-red-600">
                    <i class="fa-solid fa-xmark text-xs"></i>
                </button>
                <div id="uploadPlaceholder" class="{{ $post->image ? 'hidden' : 'flex' }} flex-col items-center justify-center">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="title_en" value="{{ old('title_en', $post->title_en) }}" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-acmi-blueprimer focus:ring-0">
                </div>
                <div class="mb-5">
                    <label class="mb-2 block text-sm font-semibold text-gray-800">Description</label>
                    <textarea name="description_en" rows="2" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-acmi-blueprimer focus:ring-0">{{ old('description_en', $post->description_en) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-semibold text-gray-800">Content</label>
                    <textarea id="acmi-editor-en" name="content_en">{{ old('content_en', $post->content_en) }}</textarea>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="title_id" value="{{ old('title_id', $post->title_id) }}" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-acmi-blueprimer focus:ring-0">
                </div>
                <div class="mb-5">
                    <label class="mb-2 block text-sm font-semibold text-gray-800">Deskripsi</label>
                    <textarea name="description_id" rows="2" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-acmi-blueprimer focus:ring-0">{{ old('description_id', $post->description_id) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-semibold text-gray-800">Isi Konten</label>
                    <textarea id="acmi-editor-id" name="content_id">{{ old('content_id', $post->content_id) }}</textarea>
                </div>
            </div>
