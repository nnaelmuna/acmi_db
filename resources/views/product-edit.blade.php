...
@section('content')
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div>
                        ...
                        <div class="relative w-full border border-gray-200 rounded-md bg-white focus-within:ring-2 focus-within:ring-[#0014A8]/20 focus-within:border-[#0014A8] transition p-1.5 flex flex-wrap gap-2 items-center">
                            <div id="category-tags" class="flex flex-wrap gap-2"></div>
                            <div class="flex-1 min-w-[120px] relative">
                                <select id="category-select" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                    <option value="" disabled selected>Pilih Kategori</option>
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->name }}">{{ $category->name }}</option>
                                    @endforeach
                                </select>
                                <div class="flex items-center justify-between pl-0 pr-2 py-1 text-sm text-gray-500 pointer-events-none">
                            ...
                            @if ($product->features)
                                @foreach ($product->features as $feature)
                                    <div class="flex items-center gap-3 bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                                        ...
                                        <button type="button" onclick="this.parentElement.remove()" class="ml-auto text-gray-300 hover:text-red-500 transition"><i class="fas fa-times text-xs"></i></button>
                                    </div>
                                @endforeach
                            @endif
                        </div>
                        <div class="flex items-center gap-3 mt-4">
                            <button type="button" onclick="addFeatureToList()" class="w-10 h-10 shrink-0 bg-white rounded-xl border border-gray-200 flex items-center justify-center text-gray-300 hover:text-[#0014A8] transition shadow-sm"><i class="fas fa-plus text-xs"></i></button>
                            <input type="text" id="feature-input" placeholder="Add Key Features.." class="flex-1 rounded-xl border border-gray-200 py-2.5 px-4 outline-none text-sm" onkeypress="if(event.key === 'Enter') { event.preventDefault(); addFeatureToList(); }">
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center gap-3">
                                ...
                                <input type="text" name="website" value="{{ old('website', $product->website) }}" placeholder="Website URL" class="w-full rounded-md border border-gray-300 py-2 px-3 outline-none focus:border-[#0014A8]">
                            </div>
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 flex items-center justify-center"><i class="fas fa-envelope text-gray-800"></i></div>
                                <input type="email" name="email" value="{{ old('email', $product->email) }}" placeholder="Company Email" class="w-full rounded-md border border-gray-300 py-2 px-3 outline-none focus:border-[#0014A8]">
                            </div>
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 flex items-center justify-center"><i class="fas fa-phone text-gray-800"></i></div>
                                <input type="text" name="phone" value="{{ old('phone', $product->phone) }}" placeholder="Company Phone" class="w-full rounded-md border border-gray-300 py-2 px-3 outline-none focus:border-[#0014A8]">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-12 flex justify-end gap-4">
                    ...
                    <button type="submit" class="px-7 py-2 rounded-lg bg-[#0014A8] text-white font-bold hover:bg-blue-900 transition shadow-lg">Update Product</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <script>
        // --- CATEGORY LOGIC ---
        let selectedCategories = @json(old('category', is_array($product->category) ? $product->category : [$product->category]));

        document.addEventListener('DOMContentLoaded', () => renderCategoryTags());

        document.getElementById('category-select').addEventListener('change', function() {
            if (this.value && !selectedCategories.includes(this.value)) {
        ...
        function renderCategoryTags() {
            ...
            tagContainer.innerHTML = ''; hiddenContainer.innerHTML = '';
            placeholder.style.display = selectedCategories.length > 0 ? 'none' : 'block';
            selectedCategories.forEach((cat, index) => {
                if(!cat) return;
                tagContainer.innerHTML += `<div class="flex items-center gap-2 bg-gray-100 px-2 py-1 rounded-md text-xs font-medium text-gray-700 border border-gray-200">${cat}<button type="button" onclick="removeCategory(${index})" class="text-gray-400 hover:text-red-500 transition"><i class="fas fa-times text-[10px]"></i></button></div>`;
                hiddenContainer.innerHTML += `<input type="hidden" name="category[]" value="${cat}">`;
            });
        }

        function removeCategory(index) { selectedCategories.splice(index, 1); renderCategoryTags(); }
        ...
        function renderPreviews() {
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                if (ex) {
                    slot.innerHTML = `<img src="{{ asset('storage') }}/${ex}" class="w-full h-full object-cover"><button type="button" onclick="removeEx(${i-1})" class="absolute top-1 right-1 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center border-2 border-white transition"><i class="fas fa-times text-[10px]"></i></button>`;
                    container.innerHTML += `<input type="hidden" name="existing_images[]" value="${ex}">`;
                } else if (newF) {
                    const reader = new FileReader();
                    reader.onload = e => {
                        slot.innerHTML = `<img src="${e.target.result}" class="w-full h-full object-cover"><button type="button" onclick="removeNew(${i - 1 - existingFiles.length})" class="absolute top-1 right-1 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center border-2 border-white transition"><i class="fas fa-times text-[10px]"></i></button>`;
                    };
                    reader.readAsDataURL(newF);
                } else {
                    slot.innerHTML = `<span class="text-[12px] font-medium text-gray-400">${labels[i-1]}</span>`;
                }
            }
        }
