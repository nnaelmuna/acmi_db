...
@section('content')
                    ...
                    @php
                        ...
                        if (isset($product->images)) {
                            if (is_array($product->images)) {
                                ...
                                $oldImages = is_array($decodedImages) ? $decodedImages : [];
                            }
                        }

                        if (empty($oldImages) && $product->image) {
                            $oldImages = [$product->image];
                        }

                        $oldImages = array_values(array_slice($oldImages, 0, 3));
                    @endphp
                        ...
                        <div class="flex flex-wrap gap-4">
                            ...
                            @for ($i = 0; $i < 3; $i++)
                                <div id="preview-slot-{{ $i + 1 }}"
                                    ...
                                    <span class="text-[12px] font-medium text-gray-400">Image {{ $i + 1 }}</span>
                                </div>
                            @endfor
                        </div>
    ...
    <script>
        ...
        // --- Image logic ---
        let existingImages = @json($oldImages);
        let uploadedFiles = [];
        ...
        function handleImageUpload(input) {
            ...
            newFiles.forEach(file => {
                if ((existingImages.length + uploadedFiles.length) < 3) {
        ...
        function renderExistingImageInputs() {
            ...
            existingImages.forEach(image => {
                container.innerHTML += `<input type="hidden" name="existing_images[]" value="${image}">`;
            });
        }
        ...
        function renderPreviews() {
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const item = combinedImages[i - 1];

                if (item) {
                    if (item.type === 'existing') {
                        slot.innerHTML = `
                            <img src="{{ asset('storage') }}/${item.value}" class="w-full h-full object-cover">
                            <button type="button" onclick="removeExistingImage(${i - 1})" class="absolute top-1 right-1 bg-red-500 text-white w-5 h-5 rounded-full text-[10px] flex items-center justify-center shadow-lg">
                        ...
                        reader.onload = e => {
                            slot.innerHTML = `
                                ...
                                <button type="button" onclick="removeNewImage(${i - 1 - existingImages.length})" class="absolute top-1 right-1 bg-red-500 text-white w-5 h-5 rounded-full text-[10px] flex items-center justify-center shadow-lg">
                                    <i class="fas fa-times"></i>
                                </button>
                            `;
                        };
